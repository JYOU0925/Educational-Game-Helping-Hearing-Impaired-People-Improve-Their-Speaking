    ...
    .then(function(stream) {
      ...
      mediaRecorder.ondataavailable = (event) => {
          if (event.data.size > 0) {
              ...
              birdY = getPitchFromBlob(event.data);
              birdY.update();
          }
      };
...
function getPitchFromBlob() {
  const basePitch = convertMapDataToY(map_data[map_data_index - 7]); // Use current map_data entry
  const randomOffset = (Math.random() - 0.5)* 50; // Small random offset
  return basePitch - randomOffset;
}
...
const pipe = {
  ...
  update: function () {
    ...
    // TODO: remove the syllables
    if (this.pipes.length && this.pipes[0].x < -this.top.sprite.width) {
      this.syllables.shift();
      this.pipes.shift();
      this.moved = true;
    }
  },
...
function showResult() {
  ...
  msgWindow.document.write 
      ...
      '<img src="/Users/apple/Documents/dissertation/dissertation/waveform_plot.png">' +
